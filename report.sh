LARGE_TEST_DOCS_DIR="./all_documents"
...
find_in_container_logs() {
    find $LARGE_TEST_DOCS_DIR -name "*.container_log" -exec grep "$@" {} \;
}

find_in_container_logs_print_file() {
    # find_in_container_logs but print the file name first
    find $LARGE_TEST_DOCS_DIR -name "*.container_log" -exec grep -H "$@" {} \;
}
...
report_composition() {
    ...
    h2 "Result of tests ran:"
    find $LARGE_TEST_DOCS_DIR -name "*.container_log" -exec egrep -h "SUCCESS|FAILURE" {} \; | sort | uniq -c
}

report_common_errors() {
    # Compute container output
    cat $LARGE_TEST_DOCS_DIR/*.container_log \
        | replace_nums_with_x \
        | container_output_exclude_expected \
...
report_failure_reasons() {
    ...
    h2 "All failures:"
    find $LARGE_TEST_DOCS_DIR -name "*.container_log" -exec grep "FAILURE" -B 2 {} \; \
        | container_output_exclude_expected \
        | replace_nums_with_x \
        | count_repeats
    ...
    h1 "timeouts"
    h2 "Summary:"
    find_in_container_logs "timed out after" | replace_nums_with_x | count_similar

    h2 "Affected files:"
    find_in_container_logs_print_file "timed out after" \
